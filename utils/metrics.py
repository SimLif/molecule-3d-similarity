...
'''
description: equal to BEDROC(alpha=80.5)
    https://www.bilibili.com/video/BV1J5411S7S8
    Structure- and Ligand-Based Virtual Screening 
    on DUD-E+: Performance Dependence on Approximations 
...
return {*}
'''
def early_enrichment(fpr, tpr, fraction=0.01):
    fpr = sorted(fpr)
    tpr = sorted(tpr)
    ...
    return tpr[index] / fpr[index]


def calculate_metrics(y_lable, y_score):
    fpr, tpr, _ = roc(y_lable, y_score)
    metrics_list = {}
    metrics_list['auc'] = auc(fpr, tpr)
    ...
    metrics_list['ER_1%'] = early_enrichment(fpr=fpr, tpr=tpr)
